...
class Player:
    ...
    def battle(self, enemy):
        ...
        なし
        """
        #「イベント発生！(enemy.name)があらわれた！」の形式で敵の出現を表示してください。
        print("イベント発生！" + enemy.name + "があらわれた！")

        #enemy.hp > 0の間繰り返す
        while enemy.hp > 0:
            #自分が敵を攻撃(self.attack(enemy)を実行)し、その戻り値がTrueならbreakしてください。
            if self.attack(enemy):
                break

            #相手が自分を攻撃(enemy.attack(self)を実行)し、その戻り値がFalseなら、
            # 「(self.name)はしんでしまった!世界は闇に包まれてしまった...」を表示し、sys.exit()を呼び出してください。
            if not enemy.attack(self):
                print(self.name + "はしんでしまった!世界は闇に包まれてしまった...")
                sys.exit()

        #「(self.name)は(enemy.name)を倒した!」を表示してください。
...
if __name__ == '__main__':
    ...
    # TODO:kevin.show_status()を実行し、バトル後の状態を表示してください。
    kevin.show_status()
