...
class Player:
    ...
    #attackメソッドを定義します。引数はself, enemyです。
    def attack(self, enemy):
        ...
        #enemy.hpからdamageを引いてください。(引いた結果をenemy.hpに再代入してください)
        enemy.hp -= damage

        if enemy.hp > 0:
            # enemy.hp > 0の場合、「(enemy.name)に(damage)のダメージを与えた!(enemy.name)のHPは(enemy.hp)です。」を表示した後、Falseを返す
            print(enemy.name + "に" + str(damage) + "のダメージを与えた!" + enemy.name + "のHPは" + str(enemy.hp) + "です。")
            return False
        else:
            # 上記以外の場合、「(enemy.name)に(damage)のダメージを与えた!(enemy.name)のHPは0です。」を表示した後、Trueを返す
...
if __name__ == '__main__':
    ...
    #kevinのattack()メソッドを、引数にenemyを与えることで、「スラスラ」を攻撃してください。
    kevin.attack(enemy)

    #enemyのattack()メソッドを、引数にkevinを与えることで、「ケビン」を攻撃してください。
    enemy.attack(kevin)
